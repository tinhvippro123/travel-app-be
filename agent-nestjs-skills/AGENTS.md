...
**Version 1.1.0 (Customized for Travel App)**
...
> Đọc kèm `PROJECT_RULES.md` để hiểu quy tắc riêng của dự án Travel App.
> Khi có xung đột giữa file này và `PROJECT_RULES.md`, **ưu tiên `PROJECT_RULES.md`**.

---

## Travel App Context

Dự án này là **Mobile App Backend** (iOS/Android) sử dụng NestJS + TypeScript.

### Mapping với Examples trong tài liệu

Khi đọc examples bên dưới, áp dụng vào dự án như sau:

| Example trong tài liệu | Tương đương trong Travel App |
|------------------------|------------------------------|
| `UsersService` / `UsersModule` | `UserService` / `UserModule` |
| `OrdersService` / `OrdersModule` | `BookingService` / `BookingModule` |
| `ProductsService` | `TourService` |
| `PaymentGateway` | Chưa có — sẽ thêm khi cần thanh toán |
| `NotificationService` | `MailService` (infrastructure) |
| `StorageService` | `CloudinaryService` (infrastructure) |

### Patterns đã áp dụng trong dự án

- **Abstract Class DI** (Rule 2.6) — Dùng `abstract class` thay `interface` cho DI tokens
- **Repository Pattern** (Rule 1.6) — Mỗi module có `IBaseRepository<T>` → concrete repo
- **Feature Modules** (Rule 1.2) — Organize by `modules/<domain>/`
- **Exception Filters** (Rule 3.3) — `HttpExceptionFilter` global
- **Guards** (Rule 4.4) — `JwtAuthGuard` + `RolesGuard` + `@Public()` + `@Roles()`
- **Input Validation** (Rule 4.5) — `ValidationPipe` global + class-validator DTOs
- **ConfigModule** (Rule 10.2) — Joi schema validation cho env vars
